...
class MidjourneyClient:
    ...
    def interact_modal(self, data, value):
            if not data:
                return
            modals = Buttoner(data["components"])
            result = self.bot.clickModal(
                data["application"]["id"],
                channelID=data["channel_id"],
                guildID=self.guild_id,
                sessionID=self.bot.gateway.session_id,
                data=modals.getModal(data, value),
            )
    ...
    def process_message(self, resp):
        ...
        print("resp.raw: " + str(resp.raw))
        if resp.event.message or resp.event.message_updated or resp.event.message_deleted or resp.raw["t"] == "INTERACTION_MODAL_CREATE":
